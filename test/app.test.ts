...
import { DiscordServer } from '../src/models/server.js'
import type { ChannelRepositoryResult, DiscordRepository, MessageRepositoryResult, ServerRepositoryResult } from '../src/services/discord.js'
...
class FakeRepository implements DiscordRepository {
  public servers: ServerRepositoryResult = { status: 'fresh', servers: [new DiscordServer('guild-1', 'Guild')] }
  ...
  public getChannels(guildId: string) {
    ...
    return this.rejectChannels ? Promise.reject(new TypeError('network unavailable')) : Promise.resolve(this.channels)
  }
...
const startAndLoad = async (app: App): Promise<void> => { await app.start(); await app.whenInitialChannelsLoaded() }

test('startup opens filtered Discord channels and creates startup once', async () => {
  const bridge = new FakeBridge(); await startAndLoad(new App(bridge, new FakeRepository()))
...
test('startup phases are ordered and startup create is called only once', async () => {
  ...
  const bridge = new FakeBridge(); await startAndLoad(new App(bridge, new FakeRepository(), phase => phases.push(phase)))
  assert.equal(bridge.startupCalls, 1)
  assert.deepEqual(phases, ['startup-container', 'startup-container', 'event-subscription', 'text-update',
...
test('BLE Bridge calls remain serialized through one queue', async () => {
  ...
  const app = new App(bridge, new FakeRepository()); await startAndLoad(app)
...
test('message empty and permission errors have distinct safe displays', async () => {
  for (const [result, expected] of [
    ...
    const bridge = new FakeBridge(); const app = new App(bridge, repository); await startAndLoad(app); bridge.emit(textEvent()); await pause()
...
test('state saves, restores valid selection, rejects broken JSON, and handles foreground events', async () => {
  const bridge = new FakeBridge(); const app = new App(bridge, new FakeRepository()); await startAndLoad(app)
  bridge.emit(textEvent(OsEventTypeList.SCROLL_BOTTOM_EVENT)); await pause()
  ...
  const restored = new FakeBridge(); restored.stored = bridge.stored; await startAndLoad(new App(restored, new FakeRepository()))
  ...
  const broken = new FakeBridge(); broken.stored = '{broken'; await startAndLoad(new App(broken, new FakeRepository()))
...
test('backend distinguishes 401 from fetch network or CORS failure', async () => {
  ...
  await assert.rejects(unauthorized.Servers(), (error: unknown) =>
  ...
  await assert.rejects(network.Servers(), (error: unknown) =>
...
test('G2 undefined CLICK_EVENT and R1 CLICK_EVENT start login once', async () => {
  for (const event of [sysEvent(undefined), sysEvent(OsEventTypeList.CLICK_EVENT, EventSourceType.TOUCH_EVENT_FROM_GLASSES_R)]) {
    const repository = new FakeRepository(); repository.channels = { status: 'login-required', channels: [], errorCode: 'DISCORD_LOGIN_REQUIRED' }
    const bridge = new FakeBridge(); const app = new App(bridge, repository); await startAndLoad(app); bridge.emit(event); await pause()
